...
public interface IPacketRepository extends JpaRepository<Packet, Long> {
    ...
    @Query(value="SELECT NEW com.clothing.management.models.DashboardCard(" +
            " p.status, " +
            "COUNT(p.status), " +
            "SUM(CASE WHEN DATE(p.date) >= DATE(:beginDate) AND DATE(p.date) <= DATE(:endDate) THEN 1 ELSE 0 END)) " +
    ...
    @Query(value ="SELECT new com.clothing.management.dto.PacketDTO(p) FROM Packet p WHERE CAST(p.id as String) LIKE %:searchField% OR  p.customerName LIKE %:searchField% OR p.customerPhoneNb LIKE %:searchField% OR p.barcode LIKE %:searchField%")
    Page<PacketDTO> findAllPacketsByField(@Param("searchField") String searchField, Pageable pageable);

    @Transactional
    @Query(value ="SELECT new com.clothing.management.dto.PacketDTO(p) FROM Packet p WHERE CAST(p.id as String) LIKE %:searchField% OR  p.customerName LIKE %:searchField% OR p.customerPhoneNb LIKE %:searchField% OR p.barcode LIKE %:searchField% OR p.packetDescription LIKE %:searchField% AND DATE(p.date) >= DATE(:beginDate) AND DATE(p.date) <= DATE(:endDate)")
    ...
    /*@Query(value ="SELECT p FROM Packet p WHERE DATE(p.date) >= DATE(:beginDate) AND DATE(p.date) <= DATE(:endDate)")
    List<PacketsPagesStatCountDTO> findAllPacketsStatsPages(@Param("beginDate") Date beginDate, @Param("endDate") Date endDate);
*/
