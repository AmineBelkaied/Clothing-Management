...
export class ListPacketsComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  constructor(private messageService: MessageService, private packetService: PacketService,
    ...
    this.statusList = ['Non confirmée','Confirmée','En rupture','En cours (1)','En cours (2)','En cours (3)','Livrée','Payée','Retour','Annulée','Injoignable','Echange'];
  }
    ...
    if (this.oldField !== packet.data[packet.field]) {
      if ((packet.field == 'city') || (packet.field == 'fbPage')) {
        ...
        this.packetService.patchPacket(packet['data'].id, updatedField)
          .subscribe((response: any) => {
            console.log(response);
            this.messageService.add({ severity: 'success', summary: 'Success', detail: 'Le champ a été modifié avec succés'});
          });
      }
    }
  }
      ...
      accept: () => {
        this.packetService.deleteSelectedPackets(selectedPacketsId)
          .subscribe(result => {
            ...
            this.packetService.allPackets = this.packetService.allPackets.filter((packet: Packet) => selectedPacketsId.indexOf(packet.id) == -1);
            this.packetsByDate = this.packetsByDate.filter((packet: Packet) => selectedPacketsId.indexOf(packet.id) == -1);
  ...
  exportExcel() {
    ...
    if(this.selectedPackets.length > 0) {
      let filteredPackets = packets.filter(packet => selectedPackets.indexOf(packet.id) > -1);
      if(filteredPackets.length > 0)
        packets = filteredPackets.slice();
    }

    packets = packets?.map((packet: any) => packet = {
        "Id" : packet.id,
  ...
  getPrice(price: any , deliveryPrice: any , dicount: any) {
     return (price != null && price != '') ? price + deliveryPrice - dicount + ' DT': '';
  }
